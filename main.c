...
void solQ3(moviesADT m, FILE * f3){
    ...
    //fprintf(f3, "year;film;votes;rating;genres\n");
    printf("year;film;votes;rating;genres\n");

    while(hasNextYear(m)){
        toBeginMostVotedMovie(m);
        nextYear(m,&year);
        for(int i = 0; hasNextTopMovie(m) && i < MAX_TOP_MOVIES; i++){
            vec = nextTopMovie(m, &votes, &rating);
            //fprintf(f3, "%d;%s;%d;%f;%s\n", year, vec[0], votes, rating, vec[1]);
            printf("%d - %d -;%s;%d;%.2f;%s\n",year, i, vec[0], votes, rating, vec[1]);
        }
    }
}
